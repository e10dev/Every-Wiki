...
class User extends LibertyModel {
  ...
  static async signUp({ email, password, username }) {
    ...
    await sendEmail({
      from: `에브리 위키 <everywikimailer@gmail.com>`,
      to: email,
      subject: '에브리 위키 이메일 인증입니다.',
      text: `이메일 인증 코드는 하루 동안 유효합니다.
${username} 사용자가 아닌 경우, 이 메일을 삭제해 주십세요.

https://${models.Setting.get('domain')}/email-confirmed?username=${encodeURIComponent(username)}&code=${confirmCode}`,
    })
      .catch((err) => next(err));
    return user;
  }

  static async passwordSendmail({ username }) {
    if (!this.validateUsername(username)) throw new Error('Invalid Username');
    const confirmCode = (await randomBytesAsync(48)).toString('hex');
    ...
    await sendEmail({
      from: `에브리 위키 <everywikimailer@gmail.com>`,
      to: email,
      subject: '에브리 위키 비밀번호 초기화입니다.',
      text: `아래 주소를 통하여 비밀번호를 초기화해 주세요.
${username} 사용자가 아닌 경우, 이 메일을 삭제해 주세요.

https://${models.Setting.get('domain')}/users/password/reset?username=${encodeURIComponent(username)}&code=${confirmCode}`,
    })
      .catch((err) => next(err));
    return email;
  }
   ...
   * @method verifyEmailConfirm
   * @async
   * @param 
   * @return {Promise<Bool>} Resolves true (confirmed) or false (unconfirmed).
   */
  verifyEmailConfirm() {
