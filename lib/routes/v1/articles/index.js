...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      await Article.createNew(reqdata);

      const getArticle = await Article.findByFullTitle(reqdata.fullTitle)
      const articleId = getArticle.dataValues.id;
      ...
      // BlockChain PUT
      if (Article.getBlockChainActive().active) {
        if (blockData.articleId != 1 && blockData.articleId != 2) {
          await requests({
            method: 'post',
            path: 'block/writeBlock',
            body: blockData 
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!article) {
        const isCreatable = await req.user.isCreatable(
          Namespace.splitFullTitle(req.params.fullTitle).namespace,
        );
      ...
      if (req.query.rev) {
        oldRev = await Revision.findByPk(req.query.rev, {
          include: [Wikitext],
        });
