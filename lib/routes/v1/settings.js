...
const {
  SET_WIKI_NAME, SET_LANGUAGE, SET_FRONT_PAGE, SET_SITE_NOTICE, SET_EMAIL, SET_LICENSE, SET_BLOCKCHAIN,
...
router.put(
  ...
  middlewares.permission(SET_EMAIL),
  [
    body('service').trim().isLength({ min: 1, max: 300 }),
  ...
  async ({
    body: {
      service, host, port, secure, user, password,
    },
  }, res, next) => {
    try {
...
router.get(
  '/blockchain',
  middlewares.permission(SET_BLOCKCHAIN),
  async (req, res, next) => {
    try {
      let blockchain = Setting.get('blockchain');
      if (!blockchain) {
        blockchain = {
          active: null,
          host: null,
...
router.put(
  ...
  middlewares.permission(SET_BLOCKCHAIN),
  [
    body('active').custom(v => typeof v === 'boolean'),
    body('host').trim().isLength({ min: 1, max: 300 }),
    body('port').custom(v => Number.isInteger(v)),
  ],
  [
